            ...
            if (r.status === 200) {
                ...
                r.data.plugins.forEach((installedPlugin: InstalledPlugin) => {
                    ...
                    tableData.body?.push([installedPlugin.name.toLowerCase(), <Button onClick={(e) => {e.preventDefault(); setShowInfo(true); getInstalledPluginInfo(installedPlugin.distro)}}><IconInfoCircle /></Button>,
                        ...
                        <Button color="tacticalRed" onClick={(e) => {e.preventDefault(); setShowCommandOutput(true); setPlugin({...plugin, 'plugin_name': installedPlugin.name, 'action': 'delete', 'plugin_distro': installedPlugin.distro}); setCommandOutputTitle(`Deleting ${installedPlugin.name}`) }}><IconCircleMinus /></Button>])
                });
